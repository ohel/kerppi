...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                case Dialect.PostgreSQL:
                    ...
                    _encapsulation = "{0}";
                    _getIdentitySql = string.Format("SELECT LASTVAL() AS collectionId");
                    break;
                case Dialect.SQLite:
                    _dialect = Dialect.SQLite;
                    _encapsulation = "{0}";
                    _getIdentitySql = string.Format("SELECT LAST_INSERT_ROWID() AS collectionId");
                    break;
                default:
                    _dialect = Dialect.SQLServer;
                    _encapsulation = "[{0}]";
                    _getIdentitySql = string.Format("SELECT CAST(SCOPE_IDENTITY()  AS BIGINT) AS [collectionId]");
                    break;
            }
        }
        ...
        /// <para>Returns a single entity by a single collectionId from table T</para>
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="connection"></param>
        /// <param name="collectionId"></param>
        /// <returns>Returns a single entity by a single collectionId from table T.</returns>
        ...
        public static TKey Insert<TKey>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (keytype == typeof (Guid))
            {
                sb.Append(";select '" + idProps.First().GetValue(entityToInsert, null) + "' as collectionId");
            }
            else
            {
        ...
        //Get all properties that are named Id or have the Key attribute
        //For Get(collectionId) and Delete(collectionId) we don't have an entity, just the type so this method is used
